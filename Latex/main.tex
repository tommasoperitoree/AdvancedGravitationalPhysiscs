...
Now the power spectrum being $\langle \Theta_{\text{obs}}(\B\ell)\Theta^*_{\text{obs}}(\B\ell)\rangle$, we have to expand this term by term. However, we can consider that since the metric potential $\Phi$ which acts as a source to our lensing potential $\phi_L$ is a perturbation of the homogeneous FLRW metric, and as a perturbation from its mean value, the potential has null average. Thus all terms with an odd number of $\phi_L$ vanish. Furthermore we are assuming that the fields are uncorrelated, i.e. $\langle \phi_L \Theta \rangle = 0$.
...
\begin{align} \begin{split}
	\langle \Theta \delta\Theta^{(2)*} \rangle +  \langle \Theta^*(\B\ell) \delta\Theta^{(2)}(\B\ell) \rangle &=  \frac{1}{2} \int \frac{d^2\ell_1 d^2\ell_2 }{(2\pi)^4} (\B\ell_1 \cdot \boldsymbol{L})(\B\ell_2 \cdot \boldsymbol{L}) \\
...
\begin{equation} \label{eq:almost_c_13}
	\langle \Theta \delta\Theta^{(2)*} \rangle +  \langle \Theta^*(\B\ell) \delta\Theta^{(2)}(\B\ell) \rangle = - \int \frac{d^2\ell_1}{(2\pi)^2} (\B\ell \cdot \B\ell_1)^2 C_{\phi\phi}(\ell_1) C(\ell)
\end{equation}
...
\begin{equation}
	\langle \Theta \delta\Theta^{(2)*} \rangle +  \langle \Theta^*(\B\ell) \delta\Theta^{(2)}(\B\ell) \rangle = -\frac{\ell^2}{2} \left[ \int \frac{d^2\ell_1}{(2\pi)^2} \ell_1^2 C_{\phi\phi}(\ell_1) \right] C(\ell) \equiv 2C^{(13)}(\ell)
\end{equation}
...
Gravitational lensing of the Cosmic Microwave Background (CMB) provides a powerful probe of the projected mass distribution in the universe. As CMB photons propagate from the last scattering surface (at recombination, $z_r \approx 1089$) to the observer, their trajectories are deflected by the gradients of the gravitational potential generated by large scale structure. In this exercise, we evaluate the impact of this remapping on the CMB power spectra, assuming a standard flat $\Lambda$CDM cosmology with $\Omega_m = 0.3$ and $H_0 = 68$ km/s/Mpc. 
...
\end{equation}
The obtained $C_{\kappa\kappa}(\ell)$ compared with the one provided in the data file is shown in the following figure
\begin{figure}[htbp]
    \centering
...
\begin{figure}[htbp]
    ...
    \caption{Fractional difference between lensed and unlensed, i.e. $(\widetilde{C}(\ell)-C(\ell)) / C(\ell)$.}
...
Let's expand the lensed $\widetilde{Q}(\B\ell)$ as we did before with the $TT$ spectrum, using \eqref{eq:Ql} and \eqref{eq:obsexpansion} and calling again $\B L = \B\ell -\B\ell_1 -\B\ell_2$,
\begin{align*}
