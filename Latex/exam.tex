...
To solve this we use a trick: going into complex field we define $z(t)$ such that $r(t) = \text{Re}[z(t)]$ and the driving force is $\text{Re}\left[-\frac{h}{2} L \omega^2 e^{i\omega t}\right]$. 
...
Substituting this into Eq. 3.72 we have to remember that our $x$ coordinate is there the $z$ coordinate, so our only non-zero second mass moment $M^{11}$ is called there $M_{33} $ and is calculated at $t_r = t + r/c$ the retarded time at which the signal is evaluated. We then obtain
\begin{align} \begin{split}
	h_+(t; \theta, \phi) &= -\frac{1}{r} \frac{G}{c^4} \ddot{M}_{11}(t_r) \sin^2 \theta \\
	&= \frac{2G\mu \omega^2}{r c^4} \sin^2 \theta \left[ C^2 \cos(2\omega t_r) + LC\cos(\omega t_r)\right]
\end{split} \end{align}
Then derivating and elevating, we have
\begin{equation}
	\dot{h}^2_+(t;\theta,\phi) \propto \omega^2\sin^4(\theta)\left[4C^2 \sin^2(2\omega t_r) + L^2 \sin^2 (\omega t_r) + 2CL\sin(2\omega t_r)\sin(\omega t)\right]
\end{equation}

Here have periodic terms with different periods, thus we have to chose the bigger of the two to carry out the integration: $T= 2\pi/\omega$. During integration, the $t_r$ can be substituted with $t$ as a shift of a constant does not change the value of the integral over a period. The first term we get is:
\begin{align}
...
\end{equation}
with $\chi^* = \chi(z^*)$. To take the matter power spectrum we were given from today $P(k,z=0)$ back to the CMB, we used the growth factor
\begin{equation}
	D(z) = \frac{H(z)}{D_0}\int_z^\infty dz' \frac{1+z'}{H(z')^3}
\end{equation}
such that $P(k,z) = P(k,0)\cdot D^2(z)$. The Hubble constant in our universe is 
\begin{equation}
	H(z) = H_0 \left[\Omega_m(1+z)^3 + \Omega_r(1+z)^4 +\Omega_\Lambda\right]^{1/2}
\end{equation}
The obtained $C_{\kappa\kappa}(\ell)$ compared with the one provided in the data file is shown in the following figure, from which we can deduce that the approximation we've done by shifting the power spectrum with the growth factor from today to the CMB becomes relevant at small $\ell$s.
\begin{figure}[htbp]
    \centering
...
\end{figure}
From the single lensing terms we can see that they actually become of order of the unlensed spectrum at high $\ell$s but since their signs are opposite their effect is minimal. Specifically, they act smoothing out the peaks of the unlensed spectrum.
\begin{figure}[htbp]
    \centering
...
\end{figure}
We also show a graph of the fractional difference (see Fig.\ref{fig:frac_TT_lensing_terms}) between the lensed and unlensed spectrum, to see that lensing brings about at most a $13\%$ smoothing of the spectrum.
\begin{figure}[htbp]
    \centering
...
\end{align*}
where we have defined $\varphi_{\ell\ell_1} := \varphi_{\B\ell-\B\ell_1}$, i.e. the angle of the vector $\B\ell-\B\ell_1$ to $\B\ell$ in the 2D flat sky, taking once again $\B\ell=(\ell,0)$ to define the $x$-axis. Similarly, $\widetilde{U}(\B\ell)$ is obtained from $\widetilde{Q}(\B\ell)$ by changing $ \cos 2\varphi_\ell\to \sin 2\varphi_\ell$ throughout.
...
This last graph shows the $BB$ mode enhanced by a factor of $400$ to better compare it with the $EE$ mode which is much stronger.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{../images/pol_lensing.png}
